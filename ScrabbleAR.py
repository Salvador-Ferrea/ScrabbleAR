...
def leerPartidasGuardadas():#<---leo el archivo de las partidas guardadas
	...
	with open('partidasGuardadas.json', 'r') as archivo:
		datos = json.load(archivo)
	return datos
def escribirArchivoFichas(dicFichas):
	with open('configuracionFichas.json', 'w') as archivo:
		json.dump(dicFichas, archivo,indent=4)
def leerArchivoFichas():
	with open('configuracionFichas.json', 'r') as archivo:
		datos = json.load(archivo)
	return datos
...
def main(args):
	...
	colDer = [
		...
		[sg.Text("Tiempo de juego:   ",font=('Helvetica', 15)), sg.Combo(values=lista,size=(3,2),background_color='lightgrey',text_color='black',readonly=True,key="tiempo",font=('Helvetica', 15))],
		[sg.Text("Nivel de juego:   ",font=('Helvetica', 15)), sg.Combo(values=("Facil", "Medio", "Dificil"),background_color='lightgrey',text_color='black',readonly=True, key="niveles",font=('Helvetica', 15))],
		[sg.Text("Elegir nivel de Top Ten:   ",font=('Helvetica', 15)), sg.Combo(values=("Facil", "Medio", "Dificil"),background_color='lightgrey',text_color='black',readonly=True, key="TopTen",font=('Helvetica', 15))],
		[sg.Text('',font=('Helvetica', 40))],
	...
	while True:
		...
		if event is None or event == 'Exit':      
			...
			if window["niveles"].Get()!=''  and window["tiempo"].Get() != '':
				...
				try:
					dicFichas= leerArchivoFichas()					
				except:
					sg.popup('no existe archivo de configuracion')
					dicFichas={}
					dicFichas['valor']=configuracionScrabble.valorPred()
					dicFichas['cantidad']=configuracionScrabble.cantidadPred()
					escribirArchivoFichas(dicFichas)
			
				modulojugador1.main(nivel,tiempo,'nuevo',dicFichas  )
				
			else:
				if window["niveles"].Get() == '':
					sg.popup('no cargo nivel')				
				if window["tiempo"].Get() == '':
					sg.popup('no cargo tiempo')				
		elif event =='configuracion de valor de fichas':
			sg.theme('DarkAmber')
			dicFichas['valor']=configuracionScrabble.main('valor de fichas',dicFichas)
			escribirArchivoFichas(dicFichas)					
		elif event =='configuracion de cantidad de fichas':
			sg.theme('DarkAmber')
			dicFichas['cantidad']=configuracionScrabble.main('cantidad de fichas',dicFichas)
			escribirArchivoFichas(dicFichas)
		elif event == 'cargar partida':
			try:
				archGuardado = leerPartidasGuardadas()
				sg.popup('Resultado:  existe partida')			
				modulojugador1.main(archGuardado['Nivel'],0,'viejo',archGuardado)				
			except:
