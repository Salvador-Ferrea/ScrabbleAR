...
def main(configuracion):
	...
	layout =[ 
			...
			[sg.Button('Modificar Letra',font=('Helvetica', 15)),sg.Text('Letra:',font=('Helvetica', 15)), sg.Combo(values=letras, key='letra',font=('Helvetica', 15),size=(3,2)),sg.Text('Valor:',font=('Helvetica', 15)), sg.Combo(values=lista, key='Valor',font=('Helvetica', 15))],
			[sg.Button('Evaluar',font=('Helvetica', 15)),sg.Button('Devolver Valores Predeterminados',font=('Helvetica', 15)),sg.Button('Ejecutar',font=('Helvetica', 15),disabled=True),sg.Button('volver a modificar',font=('Helvetica', 15),disabled=True)]
			]
		...
		if event == 'Modificar Letra':
			...
			if configuracion == 'cantidad de fichas':
				...
				if suma > 225:	
					...
					activar(window,'Evaluar','Devolver Valores Predeterminados','Modificar Letra')
					desactivar(window,'Ejecutar','volver a modificar')	
			else:
				activar(window,'Evaluar','Devolver Valores Predeterminados','Modificar Letra')
				desactivar(window,'Ejecutar','volver a modificar')
			...
			break
		elif event ==  'volver a modificar':
			activar(window,'Ejecutar','volver a modificar')
			desactivar(window,'Evaluar','Devolver Valores Predeterminados','Modificar Letra')
		
	window.close()		
	return x					
if __name__ == '__main__':
	main('cantidad de fichas')
